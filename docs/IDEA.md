...
Thunder Blessing 是一款希臘神話主題的 5×3（最大5×6）老虎機遊戲，以「滾輪持續擴展 × 符號鍊式消除 × 閃電能量蓄積」為核心遊戲循環：每次獲獎觸發 Cascade 連鎖消除並擴展滾輪列數，累積的閃電標記一旦被 Scatter 引爆，即可將整版標記格轉換為同一高賠符號，最終透過 Coin Toss 翻幣決定是否進入最高 ×77 倍率的 Free Game，提供高峰值爆發與持續張力兼具的玩家體驗。

本 GDD 套件的目標是為開發團隊提供完整且可直接執行的規格文件，使前後端工程師、企劃和 QA 均能 100% 按規格實作，最終交付 RTP 97.5%、最大獎金 30,000× 的高品質老虎機產品。
