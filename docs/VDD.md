...
| 3× | 2K / Retina 顯示器 | 512×512px（上限）| 512×512px（上限）| 512×512px（上限）|
...
> **PDD §6.3 與 VDD 差異說明**：PDD §6.3 記載整體設計目標範圍 2.5s–3.5s；VDD 各階段時長之加總最小值為 3000ms（500+800+200+1500），實作下限以 VDD 為準。若需壓縮至 2.5s，需減少飛入或加速旋轉階段時長（須動畫 QA 確認流暢感）。
...
|:----:|:---:|:---------:|:-------------:|:------:|
| 1 | ×3 | 80% | `oklch(70% 0.17 142)` #4BA03A | 連接 1→2：`oklch(78% 0.17 88)` 黃 |
| 2 | ×7 | 68% | `oklch(80% 0.18 88)` #FFD040 | 連接 2→3：`oklch(78% 0.17 88)` 黃 |
| 3 | ×17 | 56% | `oklch(72% 0.19 55)` #E8A828 | 連接 3→4：`oklch(68% 0.18 45)` 橙 |
| 4 | ×27 | 48% | `oklch(68% 0.21 40)` #E06820 | 連接 4→5：`oklch(55% 0.22 25)` 橘紅 |
| 5 | ×77 | 40% | `oklch(62% 0.22 25)` #C83018 | 連接 5：紅色電弧 |
...
| 全盤同時引爆最大粒子數 | 480 個（桌面上限，30 格 × 16 粒子均值，低於絕對上限 500）|
| 粒子速度 | 80–400px/s，外放射 |
| 粒子生命週期 | 0.3s–1.0s |
...
| Thunder Blessing 引爆 | 480 個 | 240 個 |
...
|---------|-----------|-----------|------|
| ×3 → ×7 | `oklch(70% 0.17 142)` 綠 | `oklch(80% 0.18 88)` 黃 | 進度線段色漸變 |
| ×7 → ×17 | `oklch(80% 0.18 88)` 黃 | `oklch(72% 0.19 55)` 黃橙 | |
| ×17 → ×27 | `oklch(72% 0.19 55)` 黃橙 | `oklch(68% 0.21 40)` 橙紅 | |
| ×27 → ×77 | `oklch(68% 0.21 40)` 橙紅 | `oklch(62% 0.22 25)` 深紅 | + 紅色電弧粒子 |
...
| 1.5s | 第二擊（若觸發）再次白光 | 強度降至 0.6（`rgba(255,255,255,0.6)`）（PDD §4.3 絕對 2.3s − 引爆開始 0.8s = 1.5s）|
...
設計原稿（3× @ 2560×1440 等效）
    │
    ├── 導出 3× PNG（Retina / 2K：最大 512×512px Wild，對齊 PDD §14.1 上限）
