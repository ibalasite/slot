...
Feature: Coin Toss Panel — UI Behavior
  ...
  Background:
    ...
    And the player is authenticated and the game is in Free Game state
    And the reels have expanded to 6 rows and a cascade win occurred
    And the game triggered the Coin Toss overlay
  ...
  Scenario: Tails result — "NOT THIS TIME" text and coin darkens
    ...
    And the game returns to normal payline evaluation with the newly converted symbols in position
